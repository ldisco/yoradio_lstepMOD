...
#define YOVERSION "lstepMOD v0.8.190"  /*  Версия для отображения в WebUI и Telnet. Не путать с версией прошивки, которая в EEPROM. Патч без явного запроса пользователя не поднимать. */
...
// WebUI: нижняя полоска загрузки CPU (визуально как полоса буфера #heap сверху).
// Расчёт — дельта FreeRTOS run-time counters (не-IDLE / общее время между опросами).
// Включение: #define WEBUI_CPU_BAR_ENABLE 1 в myoptions.h. По умолчанию выключено.
// Для работы нужны флаги в platformio.ini: CONFIG_FREERTOS_USE_TRACE_FACILITY,
// CONFIG_FREERTOS_GENERATE_RUN_TIME_STATS (и при необходимости USING_ESP_TIMER).
#ifndef WEBUI_CPU_BAR_ENABLE
  #define WEBUI_CPU_BAR_ENABLE 0
#endif

// ===== DLNA (этап A: каркас конфигурации) =====
// Глобальный compile-time флаг DLNA задаётся в build_flags (platformio.ini): -DUSE_DLNA.
// Здесь флаг не включаем принудительно, чтобы сохранить управляемость через сборку.

// Хост DLNA-сервера по умолчанию.
// Пустая строка означает "не задан", будет заполнен позже через UI/API.
#ifndef DLNA_HOST_DEFAULT
  #define DLNA_HOST_DEFAULT ""
#endif

// Индекс/идентификатор DLNA-контейнера по умолчанию.
// На этапе A сохраняется в конфиге для дальнейшего восстановления контекста.
#ifndef DLNA_IDX_DEFAULT
  #define DLNA_IDX_DEFAULT 0
#endif
...
// Значение 5000 = 5 секунд непрерывно стабильного потока.
#ifndef MANUAL_FACTS_STABLE_WINDOW_MS
  #define MANUAL_FACTS_STABLE_WINDOW_MS 5000
#endif
