...
constexpr uint8_t kCoverMaxScale = 2; // Ограничиваем масштаб до 1:4 для сохранения качества (1:8 может быть слишком размытым)  
...
static bool drawCoverAt(int16_t x, int16_t y, int16_t w, int16_t h, const String& coverUrl) {
  ...
  // [FIX Задача 6] Масштабирование «вписать в слот» — без обрезки.
  // Раньше условие (> w*2 && > h*2) уменьшало только когда картинка ВДВОЕ больше слота,
  // оставляя 300x300 в 220x220 без масштабирования → 40px обрезки с каждой стороны.
  // Теперь масштабируем пока картинка НЕ ВЛЕЗАЕТ в слот по любой оси.
  // TJpgDec поддерживает только степени двойки (1:1, 1:2, 1:4, 1:8).
  uint8_t scale = 0;
  uint16_t scaledW = jpgW;
  uint16_t scaledH = jpgH;
...
static const uint8_t kMaxCoverRetries = 10;  // увеличено с 3: загрузка может занять до 30 сек
