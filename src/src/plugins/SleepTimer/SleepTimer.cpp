...
static const uint16_t kSleepPresets[] = {0, 2, 30, 45, 60, 90, 120};
...
void SleepTimer::_handleExpired() {
  ...
  // Если режим ALL OFF не активен или аппаратный пин не задан, выполняем мягкий сценарий:
  // просто останавливаем воспроизведение программно.
  if (!_allOffEnabled || !hasPowerPin()) {
  ...
  //     как запасной вариант, чтобы плата не «зависала» навсегда.
    ...
    Serial.println("[SleepTimer] Нет тача и WAKE_PIN, используем таймер-пробуждение на 4 часа");
    esp_sleep_enable_timer_wakeup(4ULL * 3600ULL * 1000000ULL);
  #endif
